 ...
 *  BottomLine
 *
 *  Created by Banjo226 on 4 Dec 2015 at 9:54 am AEST
...
public class Metrics {
	...
	// checks if mcstats is up
	public boolean exists(String name) {
		try {
			HttpURLConnection.setFollowRedirects(false);
			HttpURLConnection con = (HttpURLConnection) new URL(name).openConnection();
	...
	public void findCustomData() {
		...
		Graph graph = createGraph("Player data files");

		File data = new File(plugin.getDataFolder(), "userdata");
		final File[] files = data.listFiles();

		final String name = data != null ? data.getName().replaceAll(".yml", "") : "No files";
		graph.addPlotter(new Metrics.Plotter(name) {
			@Override
			public int getValue() {
				return files.length;
			}
